    ...
    <div class="flex h-full w-full grow">
      ...
      <aside class="flex h-screen w-64 flex-col justify-between border-r p-4 sticky top-0 bg-white">
        ...
        <div class="flex flex-col gap-8">
          ...
          <div class="flex items-center gap-3 px-3">
            <div
                class="size-10 rounded-full bg-slate-900 text-white flex items-center justify-center font-bold text-lg uppercase">
              {{ userInitial }}
            </div>
      ...
      <main class="flex flex-1 flex-col bg-gray-50">
        ...
        <header class="sticky top-0 flex items-center justify-between px-10 py-3 bg-white border-b">
          ...
          <button
              @click="logout"
              class="text-red-500 hover:bg-red-50 px-4 py-2 rounded-lg transition">
            Logout
          </button>
        </header>
  ...
  async mounted() {
    ...
    try {
      const res = await axios.get(
          "http://localhost:8080/admin/dashboard-data",
      ...
      this.user = res.data?.user || { name: "Admin", role: "ROLE_ADMIN" }
