...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Add scroll effect to header.html
...
function updateCartCount() {
    ...
    if (cartCountElement) {
        // Prefer server count (DB hoặc session) => tổng số lượng
        fetch('/api/cart/count')
            ...
            .catch(() => {
                // Fallback localStorage (legacy) distinct items quantity sum
                const legacy = JSON.parse(localStorage.getItem('cart') || '[]');
                const totalCount = legacy.reduce((sum, item) => sum + (item.quantity || 0), 0);
