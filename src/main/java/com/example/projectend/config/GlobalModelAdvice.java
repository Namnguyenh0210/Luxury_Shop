 ...
 * - THÀNH VIÊN 3: (Optional) Preload categories cho header.html (cache) nếu cần ở mọi trang
 * - THÀNH VIÊN 4: (Optional) Mini thống kê (số đơn pending) hiển thị badge ở header.html admin
 * <p>
 * =============================
...
public class GlobalModelAdvice {
    ...
    private DonHangService donHangService; // Bổ sung để hiển thị badge số đơn chờ
    ...
    public void addGlobalAttributes(Model model) {
        ...
        // Bổ sung số đơn chờ xác nhận (nếu service sẵn sàng)
        if (donHangService != null) {
            model.addAttribute("pendingOrderCount", donHangService.countPending());
        } else {
            model.addAttribute("pendingOrderCount", 0L);
        }
    }
}
